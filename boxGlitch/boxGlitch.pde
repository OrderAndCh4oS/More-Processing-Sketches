...
void setup() {
  ...
  for (int i = 0; i < 64; i++) {
      ...
      for(int j = 0; j < 54; j++) {
         boxColumn.add(new Box(10, 10, bg, primary, 2, 0.006 * i * j));
      }
      boxes.add(boxColumn);
  }
  grid = new Grid(boxes);
...
class Box implements Animate {
  ...
  public Box(float width, float height, color boxFill, color boxStroke, float strokeWeight, float speed) {
    this(width, height, boxFill, boxStroke, strokeWeight);
    this.speed = speed;
  }
  
  void update() {
    angle = lerp(angle, PI*6, speed);
  }
