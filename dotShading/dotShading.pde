...
void setup() {
  PImage img = loadImage("14-breathtaking-black-and-white-animal-photos-5.jpg");
  background(#ffffff);
  size(3600, 5000);
  for (int i = 0; i < 600/6; i++) {
    for (int j = 0; j < 600/6; j++) {
      PImage newImg = img.get(i*6, j*6, 6, 6);
      float shade = extractShadeFromImage(newImg);
      println(shade);
      box = new DotBox(new Point(i*60, j*60));
...
class DotBox {
  ...
  DotBox(Point topLeft) {
    ...
    for (int i = 0; i < 12; i++) {
      for (int j = 0; j < 12; j++) {
        _dots.add(new Dot(topLeft.x() + (i * 5), topLeft.y() + (j * 5), 5));
      }
    }
  }
