...
void setup() {
  ...
  background(#0C080F);
  noStroke();
  initStems(colourSetEight, colourSetBw, 3);
  initStems(colourSetFive, colourSetBw, 2);
  saveFrame("f###.png");
}
void draw() {
  frames++;
  ...
  while (drawnCount < 40) {
    ...
    float angleRand = PI*angleRange - random(PI*angleRange*0.5);
...
void initStems(Colours foliageColours, Colours stemColours, int stemCount) {
  for (int i = 0; i < stemCount; i++) {
    Line l = new Line(new Point(250 + random(750), 250 + random(750)), 100, random(PI*2), 0.1, foliageColours, stemColours);
    drawLines.add(l);
