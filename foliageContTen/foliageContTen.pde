...
Colours foliageColours;
Colours stemColours;
Colours colours;

void setup() {
  foliageColours = colourSetFive;
  stemColours = colourSetThree;
  colours = stemColours;
  size(1500, 1500);
  background(foliageColours.bg());
  noStroke();
  for (int i = 0; i < 8; i++) {
    Line l = new Line(new Point(500 + random(500), 500 + random(500)), 100, random(PI*2));
    drawLines.add(l);
...
void draw() {
  ...
  while (drawnCount < 40) {
    ...
    float angleRand = PI*angleRange - random(PI*angleRange*1.6);
    float angle = nextLineBase.getAngle() + angleRand;
    Line nextLine = new Line(nextPoint, nextLength, angle);
