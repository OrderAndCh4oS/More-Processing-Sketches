...
void setup() {
  foliageColours = colourSetSix;
  stemColours = colourSetBw;
  colours = stemColours;
  size(1500, 1500);
  background(foliageColours.bg());
  noStroke();
  for (int i = 0; i < 3; i++) {
    Line l = new Line(new Point(750, 750), 150, random(PI*2));
    drawLines.add(l);
...
void draw() {
  ...
  while (drawnCount < 25) {
    ...
    float nextLength = nextLineBase.getLength() * 0.85 + random(0.15);
