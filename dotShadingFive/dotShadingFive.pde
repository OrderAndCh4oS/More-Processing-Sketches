...
void setup() {
  PImage img = loadImage("dawn-armfield-Ybt_9ZzcnxM-unsplash.jpg");
  background(#FCFEF7);
  size(2400, 2500);
  for (int i = 0; i < 2400/12; i++) {
    for (int j = 0; j < 2400/12; j++) {
      PImage newImg = img.get(i*15, j*15, 15, 15);
      ...
      //box.draw();
      CrossBox box;
      box = new CrossBox(new Point(i*12, j*12));
      box.setScale(1 - shade);
      box.draw();
    }
  }
...
class DotBox {
  ...
  DotBox(Point topLeft) {
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        _dots.add(new Dot(topLeft.x() + (i * 6), topLeft.y() + (j * 6), 6));
      }
    }
  }
