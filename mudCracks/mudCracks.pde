...
class Graph {
  ...
  public void draw() {
    ...
    while (i.hasNext()) {
      Edge e = (Edge)i.next();
      e.draw();
      // Todo: try to remember why this is here.
...
class MudCrackGenerator {
  ...
  MudCrackGenerator() {
    for (int i = 0; i < 3; i++) {
      ...
      preparePathTracer(graph, node, random(PI * 2), 9);
    }
  ...
  void prepareNewPathTracer() {
    ...
    preparePathTracer(graph, midPointNode, newAngle, 9);
  }
